...
def _show_new_bot_form():
    ...
    tp_enabled = st.checkbox("Take-Profit aktivieren", key="lt_new_tp")
    take_profit_pct = None
    if tp_enabled:
        st.markdown(_caption("Take-Profit (%)"), unsafe_allow_html=True)
        take_profit_pct = st.slider("", 5.0, 100.0, 20.0, 5.0,
                                   key="lt_new_tp_pct", label_visibility="collapsed") / 100
    ...
    if enable_recentering:
        ...
        _lt_rc_col1, _lt_rc_col2 = st.columns([1, 17])
        with _lt_rc_col2:
            ...
            with st.spinner("Verarbeite neue Kerzen..."):
                try:
                    ...
                    if result.get("error"):
                        ...
                        # Regime-Anzeige (kommt jetzt aus dem run_update-Result)

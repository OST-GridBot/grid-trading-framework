    ...
    if _smart_err:
        ...
        with _c1:
            if st.button("Übernehmen", use_container_width=True, key="pt_smart_apply", type="primary"):
                ...
                st.session_state["pt_new_recenter"]      = _smart.enable_recentering_up or _smart.enable_recentering_down
                st.session_state["pt_new_recenter_up"]   = _smart.enable_recentering_up
                st.session_state["pt_new_recenter_down"] = _smart.enable_recentering_down
...
def _show_bot_detail(bot: dict):
    ...
    with tab3:
        ...
        with col_b:
            ...
            _rc_up = cfg.get("enable_recentering_up",
                             cfg.get("enable_recentering", False))
            _rc_dn = cfg.get("enable_recentering_down",
                             cfg.get("enable_recentering", False))
            _rc_pct = cfg.get('recenter_threshold', 0) * 100
            if _rc_up and _rc_dn:
                st.markdown(f"- **Recentering:** Aktiv (Up + Down, {_rc_pct:.0f}%)")
            elif _rc_up:
                st.markdown(f"- **Recentering:** Aktiv (nur Up, {_rc_pct:.0f}%)")
            elif _rc_dn:
                st.markdown(f"- **Recentering:** Aktiv (nur Down, {_rc_pct:.0f}%)")
            else:

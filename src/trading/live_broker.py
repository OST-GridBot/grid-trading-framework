...
class LiveBroker:
    ...
    def execute_market_sell_real(
        self,
        amount_coin:     float,
        client_order_id: Optional[str] = None,
    ) -> dict:
        """
        Phase Live-4.4 (L-12): Echte MARKET-SELL-Order analog zu
        execute_market_buy_real, aber mit Coin-Menge statt USDT-Betrag.
        Wird vom LiveRunner.final_sell_all() beim User-getriebenen
        "Position glattstellen" aufgerufen.

        Quantity wird via _format_quantity (Decimal + stepSize) gerundet
        damit der LOT_SIZE-Filter eingehalten ist.

        Args:
            amount_coin    : Zu verkaufende Coin-Menge
            client_order_id: Optional, wird generiert wenn None

        Returns dict (gleiches Schema wie execute_market_buy_real, mit
        side="SELL" und ohne amount_usdt):
            {
                "client_order_id":  ...,
                "binance_order_id": ...,
                "symbol":           ...,
                "side":             "SELL",
                "amount_coin":      angefragte Coin-Menge,
                "exec_price":       gewichteter avg-price,
                "exec_qty":         total qty der fills,
                "commission":       total commission (echt, L-5-Logik),
                "commission_asset": Asset oder "mixed" oder None,
        ...
        try:
            qty_str = self._format_quantity(amount_coin)
        except ValueError as e:
            return {
                "client_order_id":  client_order_id,
                "binance_order_id": "",
